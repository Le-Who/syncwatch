...
- **Twitch Autoplay Policy DOM Proxy**: Fixed catastrophic crash loops where the Twitch iframe unmounted when toggling `controls`. The player now embeds an invisible Twitch element statically and safely attaches standard `addEventListener` hooks deep within the DOM node, bypassing buggy `react-player` wrappers to guarantee synchronization without blocking playback.
