...
- **Twitch Autoplay Policy DOM Proxy (Symptom-Free)**: Fixed catastrophic crash loops where the Twitch iframe unmounted when toggling `controls`. The player now embeds an invisible Twitch element statically (`pointer-events: none`) and safely attaches standard `addEventListener` hooks deep within the DOM node, bypassing buggy `react-player` wrappers to guarantee synchronization without blocking playback, all without injecting dangerous global `default-passive-events` polyfills.
- **Stateful TCP Reconnect Handshake**: Completely patched `"Invalid Session"` and `429 Too Many Requests` API avalanches. WebSocket micro-disconnects now intercept `socket.on("connect")` directly to immediately re-inject cached JWT bearer tokens into the Socket handshake prior to queuing mutations. Users never fall back to `guest_` status under instable networks.
