    ...
    <>
      ...
      {(() => {
        ...
        if (isActive && playback) {
          // Calculate progress using a stable reference if possible,
          // or just fallback to 0 until actual playback component handles it.
          // Since this renders progress of the active item, we rely on LivePosition
          // for accurate time. For the progress bar, we calculate statically.
          const elapsed =
            playback.status === "playing"
              ? (new Date().getTime() - playback.baseTimestamp) / 1000
              : 0;
          const currentPos =
...
export default function Playlist() {
  ...
  const controlMode = room?.settings.controlMode;
  const handlePlay = useCallback(
    (itemId: string) => {
      if (!canEdit && controlMode !== "hybrid") return;
      sendCommand("set_media", { itemId });
    },
    [canEdit, controlMode, sendCommand]
  );
              ...
              return (
                ...
                >
                  <PlaylistItem
                    ...
                    isActive={isActive}
                    canEdit={canEdit}
                    playback={isActive ? room.playback : undefined}
                    onPlay={handlePlay}
