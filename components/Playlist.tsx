...
// Extracting PlaylistItem as a memoized component prevents unnecessary
// re-renders of the entire playlist when the parent state updates,
// significantly improving rendering performance for large playlists.
const PlaylistItem = React.memo(({
  ...
  useEffect(() => {
    if (currentMediaId === item.id && playback?.status === "playing") {
      const timer = setInterval(() => setNow(Date.now()), 1000);
      return () => clearInterval(timer);
    }
  }, [currentMediaId, item.id, playback?.status]);
  return (
    <Reorder.Item
      value={item}
      layout
      ...
      className={`rounded-theme flex items-center border-2 p-2.5 transition-all relative overflow-hidden ${
          ...
          if (currentMediaId === item.id) {
            const elapsed =
              playback.status === "playing"
                ? (now - playback.baseTimestamp) / 1000
                : 0;
            currentPos =
