...
function bench(cmdCount) {
  ...
  for (let iter = 0; iter < ITERATIONS; iter++) {
    ...
    // Using an object dictionary for indexing, conditionally computed
    ...
    const getIndex = () => {
      if (!playlistIndex) {
        ...
        for (let i = 0; i < room.playlist.length; i++) {
          ...
          playlistIndex[item.id] = { item, index: i };
        }
      }
    ...
    for (const cmd of commands) {
      ...
      const activeEntry = index[room.currentMediaId];
      const targetEntry = index[cmd.payload.itemId];
      room.currentMediaId = cmd.payload.itemId;
    }
  }
  const timeDictIndex = performance.now() - start;

  console.log(`Commands per batch: ${cmdCount}`);
  console.log(`  find: ${timeFind.toFixed(2)}ms`);
  console.log(`  dictIndex: ${timeDictIndex.toFixed(2)}ms`);
}
