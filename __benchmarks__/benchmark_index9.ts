...
function bench() {
  ...
  for (let iter = 0; iter < ITERATIONS; iter++) {
    ...
    for (const cmd of commands) {
      const activeIdx = getPlaylistIndex()[room.currentMediaId];
      const activeItemSet =
      ...
      room.currentMediaId = cmd.payload.itemId;
      const targetIdx = getPlaylistIndex()[cmd.payload.itemId];
  ...
  console.log(
    `improvement: ${(timeFind / timeLazyIndexRecord).toFixed(2)}x faster`,
  );
}
