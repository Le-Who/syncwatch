...
function bench(cmdCount) {
  ...
  for (let iter = 0; iter < ITERATIONS; iter++) {
    ...
    // Using a map only when needed, avoiding unconditional precomputation
    ...
    for (const cmd of commands) {
      // In set_media, we need two lookups. Is creating map worth it? Let's use single pass for finding
      let activeItem, activeIndex, targetItem, targetIndex;
      for (let i = 0; i < room.playlist.length; i++) {
        const item = room.playlist[i];
        if (item.id === room.currentMediaId) {
          activeItem = item;
          activeIndex = i;
        }
        if (item.id === cmd.payload.itemId) {
          targetItem = item;
          targetIndex = i;
        }
