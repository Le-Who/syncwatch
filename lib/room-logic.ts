...
export function applyRemoveItem(
  ...
  if (room.currentMediaId === payload.itemId) {
    ...
    room.playback.status =
      room.playback.status === "playing" ? "playing" : "paused";
    // ⚡ Bolt: Use direct O(1) index access instead of O(N) Array.find() since we know it's the first element
...
export function applyVideoEnded(
  ...
  // ⚡ Bolt: Reuse the index from findIndex to get the active item in O(1) instead of an extra O(N) Array.find()
